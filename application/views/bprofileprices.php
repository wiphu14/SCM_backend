...
<body id="page-top">
  ...
  <script type="text/javascript">
    ...
    function getDataStore() {
      $.get('<?= site_url('welcome/gets_profile_price_json'); ?>', function(response) {
    ...
    function clearForm() {
      ...
      $("#car_type_id option")[0].selected = true;
      $("#h").val("");
      $("#price").val("");
    ...
    function update_for_member( /** int */ id, /** string */ v) {
      ...
      if (cc) {
        ...
        $.post('<?= site_url('welcome/update_for_member_json'); ?>', {
          'id': id,
          'for_member': v
        }, function(response) {
          if (response.success) {
    ...
    */
    function confirmDel( /** int */ el) {
      ...
      if (cc) {
        $.post("<?= site_url("welcome/delete_profile_price_json"); ?>", {
          "id": id
        }, function(response) {
          if (response.success) {
            getDataStore();
